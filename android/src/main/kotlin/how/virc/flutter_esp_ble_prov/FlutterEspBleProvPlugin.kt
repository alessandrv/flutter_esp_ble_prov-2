...
class Boss {
  ...
  var bleScanEventSink: EventChannel.EventSink? = null

    fun startBleScan(arguments: Any?, events: EventChannel.EventSink?) {
        bleScanEventSink = events
        val args = arguments as? Map<String, Any>
        val prefix = args?.get("prefix") as? String ?: ""
        bleScanner.startScan(prefix)
    }
    ...
    fun startScan(prefix: String) {
        ...
        boss.espManager.searchBleEspDevices(prefix, object : BleScanListener {
            override fun onPeripheralFound(device: BluetoothDevice?, scanResult: ScanResult?) {
                ...
                boss.bleScanEventSink?.success(deviceName)
            }
             override fun scanStartFailed() {
                boss.bleScanEventSink?.error("E1", "Scan failed to start", null)
            }
            override fun scanCompleted() {
                boss.d("searchBleEspDevices: scanComplete")
                boss.bleScanEventSink?.endOfStream()
            }

            override fun onFailure(e: java.lang.Exception?) {
                boss.d("searchBleEspDevices: onFailure")
                boss.bleScanEventSink?.error("E1", "Scan failed", e?.message)
            }
...
class FlutterEspBleProvPlugin : FlutterPlugin, MethodCallHandler, EventChannel.StreamHandler, ActivityAware, PluginRegistry.ActivityResultListener {
...
override fun onListen(arguments: Any?, events: EventChannel.EventSink?) {
    Log.d(logTag, "onListen: $arguments")
    boss.startBleScan(arguments, events)
}
...
override fun onCancel(arguments: Any?) {
  ...
  override fun onAttachedToEngine(binding: FlutterPlugin.FlutterPluginBinding) {
    ...
    eventChannel = EventChannel(binding.binaryMessenger, "flutter_esp_ble_prov/scanBleDevices")
    eventChannel.setStreamHandler(this) // Set the StreamHandler
...
override fun onDetachedFromEngine(binding: FlutterPlugin.FlutterPluginBinding) {
    ...
    eventChannel.setStreamHandler(null) // Remove the StreamHandler
}
